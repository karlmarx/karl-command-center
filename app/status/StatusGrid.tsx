          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
            {subs.map((s) => (
              <li key={s.name}>
                <a
                  ...
                  className="flex items-center justify-between gap-3 px-4 py-4 rounded-xl bg-zinc-950 border border-zinc-900 active:bg-zinc-900 active:border-emerald-500/40 transition-colors min-h-[64px]"
                >
                  <div className="flex flex-col min-w-0">
                    <span className="text-zinc-100 text-base font-medium truncate">{s.name}</span>
            ...
            {loading && subs.length === 0 && (
              Array.from({ length: 6 }).map((_, i) => (
                <li key={`skel-${i}`} className="h-[64px] rounded-xl bg-zinc-950 border border-zinc-900 animate-pulse" />
              ))
            )}
          </ul>
        )}
